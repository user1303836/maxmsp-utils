...
4. Metadata follows on outlet 2: `count`, `lowest`, `highest`, `single` (if cluster size is 1), and `done`.
...
1. `done` - signals completion
2. `highest <pitch>` - highest pitch in cluster
3. `lowest <pitch>` - lowest pitch in cluster (same as first element when sorted)
4. `count <n>` - number of notes in cluster
5. `single` - only emitted if cluster contains exactly one note
...
60 100, 64 100  ->  outlet 1: cluster 60 64
                    outlet 2: count 2, lowest 60, highest 64, done
...
72 100          ->  outlet 1: cluster 72
                    outlet 2: count 1, lowest 72, highest 72, single, done
```
